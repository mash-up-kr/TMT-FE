...
- `AppChrome`이 내비가 있는 콘텐츠 슬롯에만 `scroll-under-navigation`을 붙인다. `ScreenLayout`과 화면은 내비 inset을 직접 처리하지 않는다. AppChrome 밖 프리뷰는 `PreviewBottomNavScreenLayout`이 같은 슬롯을 소유한다.
