...
- `ScreenLayout`의 기본 본문은 `scroll-under-navigation`으로 그 높이를 padding으로 되돌린다. 자체 스크롤을 쓰려고 `bodyScrollable={false}`로 바꾼 화면은 스크롤 소유 요소에 직접 붙인다. 피드처럼 콘텐츠 표면을 내브 뒤까지 채우는 화면도 자체 스크롤을 소유하고, 목록·로딩·빈 상태 각각의 콘텐츠에 하단 여백을 적용한다. 당겨서 새로고침하는 자체 스크롤에는 `RefreshableScrollArea`를 사용한다. 이 컴포넌트는 스크롤·당김 동작만 소유하고 내비게이션 여백은 결정하지 않는다. 지도처럼 화면을 채우는 표면은 여백 없이 내브 뒤까지 연장한다.
- 피드·그룹의 검색창과 필터는 새로고침 스크롤 영역 밖에 둔다. 검색 영역은 스크롤·당김 중에도 고정하고 카드 영역만 아래로 움직인다.
