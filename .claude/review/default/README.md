...
| 구분 | 동작 |
|---|---|
| PR 생성 | 일반 리뷰를 자동 실행합니다. |
| PR 재오픈 | 일반 리뷰를 자동 실행합니다. |
| Ready for review 전환 | 일반 리뷰를 자동 실행합니다. |
| 추가 commit | 자동 재리뷰하지 않습니다. |
| 수동 재리뷰 | PR 코멘트에 `@mashong-ai 리뷰`를 입력합니다. |
| Draft PR | 작업 요약만 남기고 라인 코멘트는 보류합니다. |
