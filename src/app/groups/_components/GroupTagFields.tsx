...
import type {
  ...
  GroupTagSelection,
  GroupTagSheet,
} from "../_model/groupTag";
import { FilterOption } from "./FilterOption";

type GroupTagFieldsProps = Readonly<{
...
export function GroupTagFields({
  ...
  const { options, status } = tagOptionsState;
  ...
  const selectedCategory = options.categories.find((option) => option.id === foodCategoryId);
  const selectedRegions = options.regions.filter((option) => regionIds.includes(option.id));
...
type TagSelectFieldProps = Readonly<{
  label: string;
  value?: string;
  placeholder: string;
  onClickAction: () => void;
...
type TagOptionsContentProps = Readonly<{
