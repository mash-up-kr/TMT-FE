...
/**
 * 티켓이 있는 사람이 가입 팝업에서 `가입하기`를 누른 뒤, 가입과 함께 공유할 리뷰를 고르는 화면
 * (시안 1046:5297). 가입은 여기서 일어난다 — 공유는 가입 요청에 실려야 해서(TMT-241) 화면이
 * 가입까지 맡는다.
 *
 * 닫기와 X는 시안 주석대로 "이전 화면으로 이동"이다. 팝업에서 이미 가입을 택한 뒤이므로 공유
 * 없이 가입하고 그룹으로 돌아간다. 가입 자체를 접는 길은 브라우저 뒤로가기다.
 */
        ...
        {items.map((item, index) => (
          ...
          >
            <span className="flex items-center py-ds-4">
              {/* 시안의 체크박스는 16px이라 터치 영역이 부족하다. 가상 요소만 32까지 넓혀 레이아웃은
                  그대로 둔다 (이어쓰기 선택 화면의 라디오와 같은 방식). */}
              <Checkbox
                value={item.reviewId}
